...
### 同步H5和小程序的登录态
一般在小程序登录后把sessionKey作为一个类似token的参数携带到每次h5请求中，后端校验此sessionKey的有效性，没过期就会创建一个H5应用的cookie，每次请求校验cookie和sessionKey是否有效（匹配有效，时间有效）如果过期，会重新走小程序流程的登录生成一个新的sessionKey和新的cookie，然后同步到h5和小程序上，yi'ci
